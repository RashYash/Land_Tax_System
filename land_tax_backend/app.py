...
collection = db["historical_taxes"]
...
def train_model():
    ...
    cat_features = ['district', 'zone', 'soil_type', 'land_usage', 'irrigation', 'accessibility']
...
def predict():
    ...
    for col in feature_columns:
        if col not in df.columns:
            df[col] = "" if col in ['district','zone','soil_type','land_usage','irrigation','accessibility'] else 0
